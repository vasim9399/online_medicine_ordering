		...
		<div class="card rounded-0">
			<div class="card-header py-1 text-center">
				<div class="card-tools">
					<button class="btn btn-primary btn-sm bg-gradient-primary rounded-0" type="button" id="new_entry"><i class="fa fa-plus-square"></i> New Stock</button>
							...
							?>
								<tr class="<?= !empty($row['expiration']) && (strtotime($row['expiration']) <= strtotime(date("Y-m-d"))) ? "text-danger" : "" ?>">
									...
									<td class="p-1 text-center align-middle">
										<div class="btn-group">
											<button class="btn btn-flat btn-xs bg-gradient-primary edit_stock" title="Edit Stock" type="button" data-id='<?= $row['id'] ?>'><i class="fa fa-edit text-sm"></i></button>
											<button class="btn btn-flat btn-xs bg-gradient-danger delete_stock" title="Delete Stock" type="button" data-code='<?= $row['code'] ?>' data-id='<?= $row['id'] ?>'><i class="fa fa-trash text-sm"></i></button>
										</div>
									</td>
								</tr>
							<?php endwhile; ?>
